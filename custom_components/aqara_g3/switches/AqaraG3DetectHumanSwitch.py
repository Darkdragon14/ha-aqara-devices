...
class AqaraG3DetectHumanSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def __init__(self, coordinator: DataUpdateCoordinator, did: str, api):
        ...
        self._did = did
        self._api = api
        self._attr_unique_id = f"{did}_detect_human_active"
    ...
    async def async_turn_on(self, **kwargs):
        payload = {"data": {"humans_track_enable":"1","human_detect_enable":"1","pets_track_enable":"0","ptz_cruise_enable":"0","gesture_detect_enable":"0"}, "subjectId": self._did}
    ...
    async def async_turn_off(self, **kwargs):
        payload = {"data": {"humans_track_enable": 0}, "subjectId": self._did}
        data = await self._api.res_write(payload)
        if str(data.get("code")) != "0":
            raise Exception(f"Aqara API error: {data}")
        await self.coordinator.async_request_refresh()